...
ForEach ($computer in (Get-ADComputer -Filter *)) {
Get-WmiObject Win32_OperatingSystem -ComputerName $computer.Name | select csname, @{LABEL='LastBootUpTime';EXPRESSION={$_.ConverttoDateTime($_.lastbootuptime)}} | Export-Csv C:\Users\users\documents\UptimeReport.csv -NoTypeInformation -Encoding UTF8 -Append
}
